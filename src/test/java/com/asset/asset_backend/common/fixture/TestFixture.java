...
public class TestFixture {
    ...
    public static User createUser(String loginId, String encodedPassword) {
        return User.createUser(loginId, encodedPassword);
    }

    public static Asset createAsset(User user) {
        return Asset.createAsset("청약저축", "본인", 10_000_000L,
                AssetType.SAVINGS, 100_000L, 15, null, false, 1, user);
    }

    public static Asset createLinkedAsset(User user) {
        return Asset.createAsset("KB증권", "본인", 5_000_000L,
                AssetType.INVESTMENT, null, null, null, true, 2, user);
    }

    public static Asset createAsset(User user, int sortOrder) {
        return Asset.createAsset("자산" + sortOrder, "본인", 1_000_000L,
                AssetType.SAVINGS, null, null, null, false, sortOrder, user);
    }

    public static Debt createDebt(User user) {
        return Debt.createDebt("신용대출", "본인", 5_000_000L,
                AssetType.SAVINGS, 100_000L, 15, "생활비", null, user);
    }
    ...
    public static AssetDailySnapshot createSnapshot(User user, LocalDate date) {
        return AssetDailySnapshot.createSnapshot(user, date,
                10_000_000L, 2_000_000L, 3_000_000L, 5_000_000L, 5_000_000L);
    }

    public static MemberConfig createMemberConfig(User user, ConfigType configType, String value) {
        return MemberConfig.create(user, configType, value);
    }

    public static RefreshToken createRefreshToken(User user, String token) {
        return RefreshToken.createRefreshToken(user, token,
